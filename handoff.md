...
- Выполнено: 2 из 16 MVP-итераций; итерация 3 реализована и ожидает проверки в Unity
...
> [!IMPORTANT]
> **Текущая задача — итерация 3: Player movement.**
>
> Реализация завершена. Перед коммитом нужны импорт/компиляция и Edit Mode-тесты в Unity Editor. Итерацию 4 до этого не начинать.
...
3. Перед handoff проверяются diff, компиляция в Unity и релевантные автоматические тесты. Standalone build и ручные проверки, включая Play Mode, выполняет разработчик, если он явно не попросил иное.
...
### 3. Player movement — UNITY CHECK PENDING
...
Локальная проверка: runtime и test assembly компилируются с Unity source generators; 4 test-метода проходят через Unity Mono. Полный импорт и Test Runner в Unity Editor не запускались.

Критерий готовности: проект чисто импортируется и компилируется в Unity, 4 Edit Mode-теста проходят. Ручную проверку WASD-движения выполняет разработчик.

Плановый коммит: `feat(player): add WASD movement`
...
- [ ] **4. Enemy spawning**
  - Создать Enemy prefab и baker.
  - Добавить `SpawnConfig`.
  - Реализовать spawn по радиусу и с лимитом через ECB.
  - Коммит: `feat(spawn): add configurable enemy spawning`
